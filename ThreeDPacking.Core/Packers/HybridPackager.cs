...
namespace ThreeDPacking.Core.Packers
{
    ...
    public class HybridPackager : IPackager
    {
        ...
        private int GetGroupMaxHeight(List<BoxItem> group)
        {
            ...
            {
                // ★★★ 关键修改：取物品可能的**最大**高度，而不是最小高度
                // 这样层高度足够大，可以容纳该组所有物品的任何旋转方向
                int itemHeight = item.Box.StackValues.Max(sv => sv.Dz);
        ...
        {
            // ★★★ 关键修改：使用3D极值点计算，允许在层内向上堆叠
            var pointCalc = new PointCalculator3D();
            pointCalc.ClearToSize(container.LoadDx, container.LoadDy, container.LoadDz);

            // 创建当前层的起始点（从层底部开始）
            var levelStartPoint = new ExtremePoint(
                0, 0, levelOffset,
                container.LoadDx - 1, container.LoadDy - 1,
                container.LoadDz - 1);  // Z方向上限是容器顶部，不是层顶部
            pointCalc.SetPoints(new List<ExtremePoint> { levelStartPoint });
            pointCalc.Clear();
            ...
            while (remainingWeight > 0 && remainingVolume > 0 && !source.IsEmpty)
            {
                ...
                {
                    // 第一个物品：优先选底面积最大的
                    placement = GetFirstPlacementFromGroup3D(
                        source, pointCalc, result, groupItemIds,
                        remainingWeight, remainingVolume, levelOffset, stack.Placements, layerHeight);
                ...
                {
                    // 后续物品：贪心选择最佳位置
                    placement = GetBestPlacementFromGroup3D(
                        source, pointCalc, result, stack, groupItemIds,
                        remainingWeight, remainingVolume, levelOffset, layerHeight);
                ...
                // 清理无法放入的物品
                CleanupOversizedItems3D(source, remainingWeight, remainingVolume, pointCalc);
            }
        ...
        /// <summary>
        /// 3D版本：获取组内第一个放置位置（允许向上堆叠到容器顶部）
        /// </summary>
        private Placement GetFirstPlacementFromGroup3D(
            BoxItemSource source,
            IPointCalculator calculator,
            Container container,
            HashSet<string> groupItemIds,
            ...
            for (int pi = 0; pi < pointCount; pi++)
            {
                ...
                for (int bi = 0; bi < source.Size; bi++)
                {
                    ...
                    foreach (var sv in rotations)
                    {
                        ...
                        // ★★★ 关键修改：不再限制层高度，允许向上堆叠到容器顶部
                        // 只检查是否超出容器边界
                        // if (placement.AbsoluteEndZ >= levelOffset + layerHeight)
                        //     continue;
                        ...
                        // 支撑检查
                        if (placement.Z > 0 && !HasSufficientSupport(placement, existingPlacements))
                            continue;
        ...
        /// <summary>
        /// 3D版本：获取组内最佳放置位置（允许向上堆叠到容器顶部）
        /// </summary>
        private Placement GetBestPlacementFromGroup3D(
            BoxItemSource source,
            IPointCalculator calculator,
            Container container,
            PackStack stack,
        ...
        /// 3D版本：清理无法放入的物品
        /// </summary>
        private void CleanupOversizedItems3D(BoxItemSource source, int remainingWeight, long remainingVolume, IPointCalculator pointCalc)
        {
            if (source.IsEmpty)
                return;
